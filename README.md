...
## 가장 쉬운 방법 — 파일 하나로 전부 (다운로드 → 빌드)

배포용 **보호 exe**를 처음부터 만들려면 `BUILD.bat` **하나만** 있으면 됩니다.
저장소를 미리 받을 필요도, Python 을 미리 깔 필요도 없습니다.

1. `BUILD.bat` 을 내려받아 더블클릭
2. 알아서 진행됩니다 — Python 자동 설치 → 소스 다운로드 → 의존성 설치 →
   난독화 → 빌드
3. 끝나면 **바탕화면에 `DealSiteNewsClipper.exe`** 가 생깁니다

> Python 이 없던 PC라면, 자동 설치 후 "창을 닫고 다시 실행하세요" 안내가
> 한 번 나옵니다. BUILD.bat 을 다시 더블클릭하면 이어서 끝까지 진행됩니다.
> 전체 5~10분 정도 걸립니다.

만든 exe 는 다른 PC 에 복사만 하면 바로 실행됩니다 (Python 불필요).
